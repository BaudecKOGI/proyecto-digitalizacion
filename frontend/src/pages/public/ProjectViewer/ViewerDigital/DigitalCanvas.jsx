...
export default function DigitalCanvas({ proyecto, sidebarOpen, toggleSidebar }) {
  ...
  const [liked, setLiked] = useState(() => {
    if (typeof window !== "undefined" && proyecto?.id) {
      const likes = JSON.parse(
        localStorage.getItem("liked_digital_projects") || "[]"
      );
  ...
  const handleLike = async () => {
    ...
    const key = "liked_digital_projects";
    ...
    if (!liked) {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/metricas/por-proyecto/${id}/unlike/`,
          { method: "POST" }
        );
        if (res.ok) {
          const data = await res.json();
  ...
  const [sharesCount, setSharesCount] = useState(
    proyecto?.compartidos_totales || 0
  );
  ...
  const executeShareAPI = () => {
    if (proyecto?.id) {
      fetch(`${API_BASE}/metricas/por-proyecto/${proyecto.id}/share/`, {
        method: "POST",
      }).catch(() => {});
  ...
  const shareWhatsApp = () => {
    window.open(
      `https://api.whatsapp.com/send?text=¡Mira este proyecto digital! ${encodeURIComponent(window.location.href)}`,
      "_blank"
    );
    executeShareAPI();
    handleShareClose();
