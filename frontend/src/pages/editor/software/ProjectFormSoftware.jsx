...
export default function ProjectFormSoftware() {
  ...
  const [formData, setFormData] = useState({
    ...
    categoria: '',
    ods_ids: [],   // <-- Ahora es un array de IDs (múltiple)
    url_repositorio: '',
    url_demo_live: '',
    tecnologias: []
  });
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        ...
        // Si hay ID, cargar el proyecto para edición
        if (id) {
          const proyectoData = await fetchProyectoSoftwareById(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/editor/software/proyectos', { 
        state: { message: id ? "¡Proyecto actualizado exitosamente!" : "¡Proyecto creado exitosamente!" } 
      });
    } catch (err) {
      console.error("Error al guardar el proyecto:", err);
      setError(err.message || "Error al guardar el proyecto. Revisa los datos.");
    } finally {
        ...
        <div className="bg-[var(--panel)] p-6 md:p-10 rounded-2xl border border-[var(--line)] shadow-sm">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[var(--text-main)]">
              {id ? 'Editar Proyecto Digital' : 'Nuevo Proyecto Digital'}
            </h2>
            <p className="text-sm text-[var(--text-muted)] mt-1">
              {id ? 'Modifica la información técnica, repositorio, demo o archivos multimedia.' : 'Completa la información del nuevo proyecto de software.'}
            </p>
          </div>
