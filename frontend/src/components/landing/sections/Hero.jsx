...
function Typewriter({ text = "Todo en un solo lugar.", typingSpeed = 100, deletingSpeed = 75, pauseTime = 3200 }) {
  ...
  return (
    <span className="inline-flex items-center whitespace-nowrap">
      <span>{displayed}</span>
      <span className="ml-[3px] inline-block w-[3px] h-[0.9em] bg-[#6802C1] animate-pulse" />
    </span>
  );
}
  ...
  return (
    <section id="hero" className="relative flex min-h-screen items-center overflow-hidden bg-[#090D16] pt-24 pb-16">
      <video
        ...
        src="/assets/heros/hero-video.mp4"
      />
      ...
      <div className="relative z-20 mx-auto w-full max-w-[1400px] px-6 md:px-12 lg:px-20">
        ...
        >
          ...
          {/* Fila Inferior: Botón alineado más arriba junto a la descripción */}
          <motion.div variants={itemVariants} className="mt-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
            {/* Botón de acción */}
            <div>
              <a
                href="#duality"
                className="group inline-flex items-center gap-4 px-8 py-4 bg-[#6802C1] text-white font-sans text-sm font-bold uppercase tracking-wider transition-colors hover:bg-[#5A00AA] hover:text-white"
              >
